...
GroupConsumer.prototype._updateSubscriptions = function (partitionAssignment) {
    ...
    return self.client.updateMetadata().then(function () {
        ...
        if (p.error || p.offset < 0) {
            opt.time = self.options.startingOffset;
        } else {
            opt.offset = p.offset + 1;
        }

        return self.subscribe(p.topic, p.partition, opt, handler).catch(function (err) {
            return self.client.error('Failed to subscribe to', p.topic + ':' + p.partition, err);
        });
    });
};

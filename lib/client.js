...
Client.prototype.findLeader = function (topic, partition, notfoundOK) {
    var self = this;
    var leaderNotfoundOK = _.isBoolean(notfoundOK) ? notfoundOK : self.options.leaderNotfoundOK;

    return self._waitMetadata().then(function () {
        var defaultR = leaderNotfoundOK ? parseInt(_.keys(self.brokerConnections)[0]) : -1;
        var r = _.get(self.topicMetadata, [topic, partition, 'leader'], defaultR);
        if (r === -1) {
            throw errors.byName('UnknownTopicOrPartition');
        }
